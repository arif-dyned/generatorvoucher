...
<script type="text/javascript">
    $(document).ready(function () {
        $('#patner').DataTable({
            serverSide: true,
            processing: true,
            ajax: "<?php echo e(route('trtempl.orlevel')); ?>",
            columns: [
                {data: 'id', orderable: false, 'title': 'No'},
                {data: 'organization_level_name', orderable: true},
                {
                    data: 'id', orderable: false, 'title': 'Action', "render": function (data, type, row) {
                    return '' +
                        '<a href="#" data-id="' + row.id + '" data-name="' + row.organization_level_name.replace(/_/g, " ") + '" name="detail">Detail |</a>&nbsp;&nbsp;&nbsp;' +
                        '<a href="#" data-id="' + row.id + '" data-name="' + row.organization_level_name.replace(/_/g, " ") + '" name="edit">Edit |</a>&nbsp;&nbsp;&nbsp;' +
            ...
            'columnDefs': [
                {
                    ...
                    'targets': 1,
                    'createdCell': function (td, cellData, rowData, row, col) {
                        var str = rowData.organization_level_name;
    ...
    function deleteOrganization(id) {
        ...
        if (r == true) {
            $.ajax({
                url: "<?php echo e(url('orlevel/delete-organization')); ?>",
                type: "POST",
                data: {
                    id: id
                },
    ...
    $(document).on('click', '[name=new]', function () {
        $('#myModals').find('.modal-title').text('New Organization Level')
    ...
    $(document).on('click', '[name=detail]', function () {
        $('#myModals').find('.modal-title').text('Detail Organization Level')
    ...
    $(document).on('click', '[name=edit]', function () {
        $('#myModals').find('.modal-title').text('Edit Organization Level')
        $('#myModals').find('[name=_id]').removeAttr('readonly')
        $('#myModals').find('[name=_name]').removeAttr('readonly')
        $('#myModals').find('button[name=save]').attr('style', '')
    ...
    function deleteOrganization(id) {
        ...
        if (r == true) {
            $.ajax({
                url: "<?php echo e(url('orlevel/delete')); ?>",
                type: "POST",
                data: {
                    id: id
                },
