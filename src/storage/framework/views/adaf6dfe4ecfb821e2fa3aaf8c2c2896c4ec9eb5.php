...
<script type="text/javascript">
    $(document).ready(function () {
        $('#patner').DataTable({
            serverSide: true,
            processing: true,
            ajax: "<?php echo e(route('trtempl.grade')); ?>",
            columns: [
                {data: 'id', orderable: false, 'title': 'No'},
                {data: 'grade_name', orderable: true},
                {
                    data: 'id', orderable: false, 'title': 'Action', "render": function (data, type, row) {
                    return '' +
                        '<a href="#" data-id="' + row.id + '" data-name="' + row.grade_name.replace(/_/g, " ") + '" name="detail">Detail |</a>&nbsp;&nbsp;&nbsp;' +
                        '<a href="#" data-id="' + row.id + '" data-name="' + row.grade_name.replace(/_/g, " ") + '" name="edit">Edit |</a>&nbsp;&nbsp;&nbsp;' +
    ...
    function deleteOrganization(id) {
        ...
        if (r == true) {
            $.ajax({
                url: "<?php echo e(url('grade/delete-organization')); ?>",
                type: "POST",
                data: {
                    id: id
                },
    ...
    $(document).on('click', '[name=new]', function () {
        $('#myModals').find('.modal-title').text('New Grade')
    ...
    $(document).on('click', '[name=detail]', function () {
        $('#myModals').find('.modal-title').text('Detail Grade')
    ...
    $(document).on('click', '[name=edit]', function () {
        $('#myModals').find('.modal-title').text('Edit Grade')
        $('#myModals').find('[name=_id]').removeAttr('readonly')
        $('#myModals').find('[name=_name]').removeAttr('readonly')
        $('#myModals').find('button[name=save]').attr('style', '')
    ...
    function deleteOrganization(id) {
        ...
        if (r == true) {
            $.ajax({
                url: "<?php echo e(url('grade/delete')); ?>",
                type: "POST",
                data: {
                    id: id
                },
